...
def _load(source: Union[Path, str]):
    ...
    """
    # Local folder case (old behavior)
    ...
    # Hugging Face repo ID case (new behavior)
    else:
        repo_id = str(source)
...
def load_all():
    """Idempotent: load both models if not loaded. Uses local relative paths."""
...
LEX: Dict[str, List[str]] = {
    "racial": ["race", "racist", "ethnic", "black", "white", "asian", "african american", "hispanic", "latino", "black women are too angry", "asian women are submissive", "all white women exhibit the 'Karen behavior'", "latina chicks are easy"],
    "sexist": ["women are", "men are", "boys will be boys", "girls can't", "women should", "feminazi", "men should", "too emotional"],
    "classist": ["poor", "rich", "lower class", "upper class", "welfare", "lazy"],
    "ageism": ["old people", "should retire", "let younger one have the job", "boomers", "over-qualified", "not as agile", "too old", "too young", "kids these days"],
    "antisemitic": ["jew", "jews", "zionist", "antisemitic", "rothschild"],
    "bullying": ["idiot", "moron", "retard", "useless", "pathetic", "trash", "die", "dick head", "jerk", "numb nuts", "pussy"],
...
def predict_implicit_explicit(text: str) -> int:
    ...
    explicit_terms = [
        # racial slurs
        "nigger", "spic", "chinc", "wetback", "darkie", "towelhead", "illegal alien",
        # sexist / misogynistic
        "bitch", "whore", "bitches and hoes", "slut", "bitch slut",
        # antisemitic
        "jew bitch", "stingy as a jew",
        # bullying / self-harm
        "you should kill yourself", "go kill yourself", "you should die",
        "you're stupid", "you're a moron", "idiot", "pathetic", "trash",
    ]
    ...
    group_terms = [
        "women", "men", "black women", "black people", "asian women", "asians",
        "jews", "immigrants", "old people", "young people", "poor people",
        "rich people", "those people", "they all", "they are",
    ]

    # NEGATIVE_STEREOTYPE_WORDS should be defined above in this file
    # (the same list we added earlier: "bad", "hostile", "submissive", etc.)
...
def assess_bias_severity(
    ...
    # --------------------------------------------------------
    # Normal logic (some signal exists)
    # --------------------------------------------------------
    top_label, max_prob = max(scores.items(), key=lambda kv: kv[1])
    implicit_explicit = predict_implicit_explicit(text)
    ...
    # Escalate certain categories and explicit language
    if top_label in SEVERE_BLOCK_CATEGORIES and severity == "medium":
        severity = "high"
        reason += " Escalated due to category being high-risk."

    if implicit_explicit == 1 and severity != "none":
        # explicit phrasing escalation
        if severity == "low":
            severity = "medium"
        elif severity == "medium":
            severity = "high"
